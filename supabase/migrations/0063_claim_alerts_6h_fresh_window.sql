-- 2026-07-31: custom notify_level='high' subscriptions were structurally silent. 'high'
-- requires importance=3 (Gemini) AND published <6h — but the matcher only looked at
-- articles first_seen in the last 30 MINUTES. Enrichment lands minutes-to-hours after
-- first_seen (hourly backfill; worse under IO starvation), so by the time an article
-- earned importance=3 its match window had closed. Widen the window to 6 hours — the
-- same freshness horizon article_tier() already enforces. Idempotency is unaffected:
-- the per-user/article NOT EXISTS, 6h cluster gate, 12h title-similarity gate, 24h
-- embedding gate and daily cap all still apply, so re-scanning the same articles every
-- tick cannot double-send. Scan cost is unchanged (bounded by the published_at<24h
-- index range, same as before).

CREATE OR REPLACE FUNCTION public.claim_alerts()
 RETURNS TABLE(alert_id uuid, user_id uuid, article_id uuid, topic text, kind text, title text, excerpt text, source_name text, cluster_id uuid, devices jsonb)
 LANGUAGE sql
 SECURITY DEFINER
...
with fresh as (
  ...
  where a.first_seen_at > now() - interval '6 hours'
